...
test('DPT10', function (t) {
    var dpt = DPTLib.resolve('DPT10');
    ...
    for (var i = 0; i < tests.length; i++) {
        ...
        t.deepEqual(converted, buf, `DPT10 formatAPDU ${JSON.stringify(val)}`);

        // forward test (raw data to value)
        converted = dpt.fromBuffer(buf);
        t.equal(converted, val, `DPT10 fromBuffer ${JSON.stringify(buf)}`);
    }
    t.end()
});
