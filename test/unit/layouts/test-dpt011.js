...
test('DPT11', function (t) {
    var dpt = DPTLib.resolve('DPT11');
    ...
    for (var i = 0; i < tests.length; i++) {
        ...
        t.deepEqual(converted, buf, `DPT11 formatAPDU ${JSON.stringify(val)}`);

        // forward test (raw data to value)
        converted = dpt.fromBuffer(buf);
        t.equal(converted.getDate(), val.getDate(), `DPT11 fromBuffer ${JSON.stringify(buf)}`);
        t.equal(converted.getMonth(), val.getMonth(), `DPT11 fromBuffer ${JSON.stringify(buf)}`);
        t.equal(converted.getFullYear(), val.getFullYear(), `DPT11 fromBuffer ${JSON.stringify(buf)}`);
    }
    t.end()
});
