...
test('DPT8 conversion', function (t) {
    ...
    for (var i = 0; i < tests.length; i++) {
        ...
        t.deepEqual(converted, buf, `DPT8 formatAPDU ${JSON.stringify(obj)}`);

        // forward test (raw data to object)
        var converted = dpt.fromBuffer(buf);
        t.equal(converted, obj, `DPT8 fromBuffer ${JSON.stringify(buf)}`);
    }
    t.end();
});