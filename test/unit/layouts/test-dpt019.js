...
test('DPT19 conversion', function (t) {
    ...
    for (var i = 0; i < tests.length; i++) {
        ...
        t.deepEqual(converted, buf, `DPT19 formatAPDU ${JSON.stringify(obj)}`);

        // forward test (raw data to object)
        converted = dpt.fromBuffer(buf);
        t.equal(converted.toString(), obj.toString(), `DPT19 fromBuffer ${JSON.stringify(buf)}`);
    }
    t.end();
});